...
// 创建新的区块
// func generateBlock(prevBlock Block) Block {
...
func calculateMerkleRoot(transactions []Transaction) string {
	...
	for _, tx := range transactions {
		txHashes = append(txHashes, calculateTransactionHash(tx))
	}
	return calculateMerkleRootFromHashes(txHashes)
}
...
func calculateMerkleRootFromHashes(hashes []string) string {
	if len(hashes) == 0 {
		return ""
	}
	if len(hashes) == 1 {
	...
	return calculateMerkleRootFromHashes(newLevel)
}
...
func syncTransaction(transaction Transaction) {
	if getShardIndex(transaction.Sender) != getShardIndex(CurrentNode.Address) {
		return
	}
...
// 计算交易属于哪个分片
func getShardIndex(address string) int {
